...
export class BackupsManagementDialogComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    if (isNil(data)) {
      throw new Error(
        "Got an empty MAT_DIALOG_DATA in BackupsManagementDialogComponent. Aborting"
      );
    }
    ...
    this.areBackupsReadyToRender$ =
      this.backupsFacade.isBackupsReadyToRender$();
    this.selectedBackup$ = this.backupsFacade.getBackupChoice$();

    this.worlds$ = this.backupsFacade.getSnapshotWorlds$();
    this.worlds$.subscribe((data: any) => {
      console.log("WORLDS LIST");
      console.log(data);
    });
    ...
    this.backupsList$.subscribe((data: any) => {
      console.log("BACKUPS LIST SUBSCRIPTION");
      console.log(data);
    });
    this.backupsDataSource$ = this.backupsList$.pipe(
      map((backups: BackupDefinition[]) => {
        const dataSource: MatTableDataSource<BackupDefinition> =
          this.dataSource;
    ...
    this.isConfirmBackupButtonDisabled$ = combineLatest([
      ...
      map(
        ([
          containerRunning,
          worldsToRestore,
          bypassRunningContainerRestrictions,
        ]) => {
    ...
    this.selectedBackup$
      ...
      .subscribe((backupChoice: BackupDefinition | null) => {
        if (backupChoice === null) {
          console.warn(
            "backupChoiceConfirmed called but backupChoice was null!"
          );
          return;
        }
        ...
        this.backupsFacade.onBackupChoiceConfirmed(
          this.containerDef,
          backupChoice,
          this.worldsToRestore$.getValue(),
          this.bypassRunningContainerRestriction$.getValue()
        );
      });
  }
