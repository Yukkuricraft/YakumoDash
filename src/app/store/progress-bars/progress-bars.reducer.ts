...
import { BackupDefProp, ContainerDefAndBackupChoiceProp, ContainerDefAndMessageProp, ContainerDefProp, backupChoiceConfirmed, backupCreationFailed, backupCreationSuccessful, createBackupButtonClicked, rollbackFailed, rollbackSuccessful } from "@app/store/backups/backups.actions";
...
export const ProgressBarsReducer = createReducer<ProgressBarsFeatureState>(
    ...
    on(
        rollbackFailed,
        rollbackSuccessful,
        (state: ProgressBarsFeatureState, { backupDef }: { backupDef: BackupDefinition | null } ) => {
    ...
    on(createBackupButtonClicked, (state: ProgressBarsFeatureState, { containerDef }: ContainerDefProp) => {
    ...
    on(
        ...
        (state: ProgressBarsFeatureState, { containerDef }: { containerDef: ContainerDefinition } ) => {
            const identifier = `Creating new backup: ${containerDef.getHostname()}`;
            let newState = {
                ...state,
                progressBars: {
