...
import { ProgressBarsFeatureState, ProgressBarsState } from "@app/store/progress-bars/progress-bars.reducer";
...
const getProgressBarsState = createFeatureSelector<ProgressBarsFeatureState>(ProgressBarsFeatureKey);
...
export const getSortedActiveProgressBars = createSelector(
  ...
  (progressBars: ProgressBarsState) => {
    let data: ProgressBarIdentifierAndTimestampProp[] = Object.entries(progressBars).map(
      ([identifier, timestamp]: [string, Date]) => {
