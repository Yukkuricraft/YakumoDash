...
import {
  ...
  ContainerDefAndBackupsListProp,
  ContainerDefAndWorldListProp,
  backupChoiceConfirmed,
  backupChoiceDeselected,
...
export const BackupsReducer = createReducer<BackupsFeatureState>(
  ...
  on(
    backupsComponentInit,
    (state: BackupsFeatureState, { containerDef }: ContainerDefProp) => {
  ...
  on(
    ...
    (
      ...
      { containerDef, backupsList }: ContainerDefAndBackupsListProp
    ) => {
      const containerHostname = containerDef.getHostname();
  ...
  on(
    ...
    (
      state: BackupsFeatureState,
      { containerDef, worlds }: ContainerDefAndWorldListProp
    ) => {
      const containerHostname = containerDef.getHostname();
  ...
  on(backupChoiceConfirmed, (state: BackupsFeatureState) => {
    console.log("Rollback initiated or backup choice confirmed");
    const containerDef = state.containerDef;
    if (containerDef === null) {
      console.warn(
        `Tried initiating rollback but state.containerDef was not set!`
      );
      return {
        ...state,
      };
    ...
    if (backupDef === null) {
      console.warn(
        `Tried initiating rollback but state.backups[containerHostname].backupChoice was not set!`
      );
      return {
        ...state,
      };
  ...
  on(
    rollbackSuccessful,
    (state: BackupsFeatureState, { backupDef }: BackupDefProp) => {
      let newState = {
        ...state,
        rollbacks: {
          ...state.rollbacks,
...
export const backupsFeatureReducer = [BackupsReducer];
